...
var StockDetailsScreen = GObject.registerClass({
  ...
  async _sync () {
    ...
    try {
      ...
      log(`📊 Details: Historical data received: ${quoteHistorical?.Data?.length || 0} data points`)

      this._isIntrayDayChart = CHART_RANGES.INTRADAY === this._selectedChartRange

      this._quoteSummary = quoteSummary

      log('📈 Details: Building UI components...')
      this._buildUI(quoteSummary, quoteHistorical)
      log('📈 Details: UI components built successfully')
      
    } catch (error) {
      log(`📈 Details: Error loading data: ${error.message}`)
      // Show some basic UI even on error
      this._buildErrorUI(error)
    }
  }

  _buildUI(quoteSummary, quoteHistorical) {
    ...
    // Only add chart components if we have historical data
    if (quoteHistorical?.Data?.length > 0) {
      log('📈 Details: Adding chart components')
      ...
      log('⚠️ Details: No historical data, skipping chart')
      // Add a message about missing chart data
      const noChartLabel = new St.Label({
        text: 'Chart data temporarily unavailable',
        style_class: 'chart-unavailable-label'
      })
      this.add_child(noChartLabel)
    }
  }
