...
const { roundOrDefault, getStockColorStyleClass, isNullOrEmpty, executeSequentially } = Me.imports.helpers.data
...
var MenuStockTicker = GObject.registerClass({
  ...
  _init () {
    ...
    this._lastValidQuotes = null  // Store last valid quotes to prevent flashing
  ...
  async _sync () {
    try {
      ...
      // Remove loading message flash - fetch data silently
      ...
      const quoteTasks = tickerBatch.map(stockItem => () => FinanceService.getQuoteSummary({
        ...stockItem,
        fallbackName: stockItem.name
      }))
      
      const quoteSummaries = await executeSequentially(quoteTasks)
      ...
      logError(new Error("💥 Ticker sync failed: " + e.message))
      this._showInfoMessage("⚠️ Failed to load stock data.")
    }
  }
  ...
  _createMenuTicker ({ quoteSummaries }) {
    ...
    // Filter out null values and stocks with errors, but show them if all failed
    const validQuotes = quoteSummaries.filter(quoteSummary => quoteSummary !== null && !quoteSummary.Error)
    const quotesToShow = validQuotes.length > 0 ? validQuotes : quoteSummaries.filter(quoteSummary => quoteSummary !== null)
    
    if (quotesToShow.length === 0) {
      this._showInfoMessage("⚠️ All stocks failed to load")
      return
    }
    
    quotesToShow.forEach((quoteSummary, index, filteredArray) => {
